...
class BramController(val p: MemCtrlParams) extends MultiIOModule {
  ...
  val regBramRspsCount = RegInit(0.U(log2Ceil(constBramRsps).W))
  val bramRspsEnq = WireInit(false.B)
  val bramRspsDeq = WireInit(false.B)
  ...
  switch(regState) {
    is(sIdle) {
      when(qBramRsps.count < (constBramRsps-2).U) {
  ...
  assert(!(qBramRsps.enq.valid && !qBramRsps.enq.ready))
  ...
  val bramRspCountNext_1 = WireInit(regBramRspsCount)
  when (bramRspsEnq) {
    bramRspCountNext_1 := regBramRspsCount + 1.U
  }
  val bramRspCountNext_2 = WireInit(bramRspCountNext_1)
  when (bramRspsDeq) {
    bramRspCountNext_2 := bramRspCountNext_1 - 1.U
  }
  regBramRspsCount := bramRspCountNext_2
}